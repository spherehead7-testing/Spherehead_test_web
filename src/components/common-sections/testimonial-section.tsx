...
export default function TestimonialSection({
    ...
    const handleNext = () => {
        ...
        setDirection("next");
        setPhase("moving");
        moveTimeoutRef.current = window.setTimeout(finishAnimation, BASE_DURATION * 2000);
    };
    ...
    /* ── MobileCard: image left (no crop) + text right ──────────── */
    const MobileCard = ({ card }: { card: (typeof testimonials)[number] }) => (
        <div className="flex w-full flex-row">
            {/* Image column — aspect-ratio box so the full portrait is shown, no cropping */}
            <div className="relative shrink-0" style={{ width: "55%" }}>
                <div
                    className="relative w-full overflow-hidden rounded-[6px] rounded-br-[18px]"
            ...
            {/* Text column — fills height of image, no overflow below */}
            <div className="flex flex-1 min-w-0 flex-col justify-between pl-3 pt-2 pb-2">
                <p className="whitespace-pre-line text-[#01030b] text-[11px] leading-[1.5]">
                    {card.quote}
                </p>
                <div>
                    <h3 className="text-[#01030b] font-semibold text-[14px] leading-tight">
                        {card.name}
                    </h3>
                    <p className="text-[#8A8B8F] text-[10px] mt-0.5">{card.role}</p>
                </div>
            </div>
        </div>
    );
        ...
        >
            <SiteContainer>
                ...
                >
                    ...
                    >
                        <div
                            ...
                            style={{ width: IMG_W, top: IMG_H, height: NAV_H }}
                        >
                            <button
                                type="button"
                        ...
                        >
                            ...
                            {phase === "moving" && movingItem && direction === "next" && (
                                <motion.div
                                    ...
                                    style={{ width: SIDE_CARD_W, height: SIDE_CARD_H }}
                                >
                                    <SideCardInner card={testimonials[movingItem.testimonialIndex]} />
                                </motion.div>
                            )}
                        </div>
                    </div>
                ...
                {/* ══════════════════════════════════════════════
                    ...
                    │ ‹  ›                      │      │      │
                    └───────────────────────────┴──────┘      │
                    Only ONE gray pill. No content below image.
                ══════════════════════════════════════════════ */}
                <div
                    className={cn(
                        "flex sm:hidden items-stretch",
                ...
                >
                    ...
                    <div className="relative flex-1 min-w-0">
                        ...
                        {phase === "moving" && (
                            <motion.div
                                initial={{ x: direction === "next" ? 280 : -280, opacity: 0 }}
                                animate={{ x: 0, opacity: 1 }}
                                transition={{ duration: BASE_DURATION * 1.2, ease: "easeOut" }}
                                className="absolute top-0 left-0 z-10 bg-white w-full"
                            >
                                <MobileCard card={activeCard} />
                            </motion.div>
                        )}
                        ...
                        {phase === "moving" && (
                            <motion.div
                                ...
                                className="absolute top-0 left-0 pointer-events-none w-full bg-white"
                        ...
                        {/* Arrows sit below image column, not below the whole card */}
                        <div
                        ...
                        >
                            <button
                                ...
                                aria-label="Previous testimonial"
                                onClick={handlePrev}
                                className="flex h-8 w-8 items-center justify-center -ml-2 text-[#01030b] transition active:text-[#0D54CA]"
                            >
                                <ChevronLeft aria-hidden="true" className="h-5 w-5" />
                            </button>
                            <button
                                ...
                                aria-label="Next testimonial"
                                onClick={handleNext}
                                className="flex h-8 w-8 items-center justify-center text-[#01030b] transition active:text-[#0D54CA]"
                            >
                                <ChevronRight aria-hidden="true" className="h-5 w-5" />
                            </button>
                        </div>
                    ...
                    {/* Single gray pill — full height of the main area */}
                    <div
                        className="relative shrink-0 self-stretch overflow-hidden rounded bg-[#f1f1f1]"
                        style={{ width: M_SIDE_W, marginLeft: M_SIDE_GAP }}
                    >
                        {/* Resting card */}
                        {(() => {
                            ...
                            return (
                                <div key={item.uid} className="absolute inset-0">
                                    <SideCardInner card={testimonials[item.testimonialIndex]} />
                                </div>
                            );
                        })()}

                        {/* "next": outgoing card slides left out of pill */}
                        {phase === "moving" && movingItem && direction === "next" && (
                            <motion.div
                                initial={{ x: 0 }}
                                animate={{ x: -(M_SIDE_W * 4) }}
                                transition={{
                                    duration: BASE_DURATION * 2,
                                    times: [0, 0.3, 0.85, 1],
                                    ease: "easeInOut",
                                }}
                                className="absolute inset-0 z-20 rounded bg-[#e9e9e9]"
                            >
                                <SideCardInner card={testimonials[movingItem.testimonialIndex]} />
                            </motion.div>
                        )}

                        {/* "prev": incoming card slides in from right */}
                        {phase === "moving" && direction === "prev" && (() => {
                            const incoming = sideStack.find(s => s.uid === incomingUid);
                            if (!incoming) return null;
                            return (
                                <motion.div
                                    key={incoming.uid}
                                    initial={{ x: M_SIDE_W + 10 }}
                                    animate={{ x: 0 }}
                                    transition={{ duration: BASE_DURATION, ease: "easeInOut" }}
                                    className="absolute inset-0 z-20 rounded bg-[#f1f1f1]"
                                >
                                    <SideCardInner card={testimonials[incoming.testimonialIndex]} />
                                </motion.div>
                            );
                        })()}
                    </div>
