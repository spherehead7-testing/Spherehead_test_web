...
export default function WorkShowcaseSection() {
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // 1. Let UPWARD scrolls on the intro screen bubble freely
      ...
      // ✨ THE FIX: Actively swallow and destroy momentum while animating!

      // This stops a "huge swipe" from queuing up a second jump.

      if (stepLockRef.current) {

        e.preventDefault();

        wheelAcc.current = 0; // Keep draining the accumulator to 0
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      if (activeIndex === lastIndex) {
        ...
        // ✨ THE FIX: Lock the footer zone for swipes too
  ...
  // ── Momentum Clamp (Cleaned up & Fixed) ──────────────────
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // ✨ THE FIX: Disable the clamp while the smooth scroll animation is running!
